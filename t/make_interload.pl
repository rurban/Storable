...
sub uuencode_it {
  my ($data, $name) = @_;
  my $bit = length(pack("p","")) == 4 ? "32bit" : "64bit";

  printf "begin %3o $bit $name\n", ord 'A';
  print pack 'u', nfreeze($data);
  print "\nend\n\n";
  
  printf "begin %3o $bit $name\n", ord 'A';
  print pack 'u', freeze($data);
  print "\nend\n\n";
}
...
lock_keys %hash, 'cperl', 'rules';
lock_hash %hash;
uuencode_it (\%hash, "Locked hash placeholder");
unlock_hash %hash;

lock_keys %hash, 'cperl';
...
my %uhash = ("\x{100}", "gone", "cperl", "rules");
delete $uhash{"\x{100}"};
